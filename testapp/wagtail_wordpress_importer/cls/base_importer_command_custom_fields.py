...
from .api_fetcher_custom_fields import WordpressCustomFieldsApiFetcher
from .logging import SimpleLog


class BaseImporterCustomFieldsCommand(BaseCommand):
    '''
    The base commmand class for importing wordpress api endpoints for custom fields
    For each of the endpoints you need to import sublclass BaseImporterCustomFieldsCommand
    ...
    ''' e.g. 'wp-json/custom_fields/v1/groups' '''
    api_endpoint = None
    ...
    def handle(self, *args, **options):
        ...
        for subsite in self.api_sub_sites:
            ...
            self.fetcher = WordpressCustomFieldsApiFetcher(url)
    ...
    def get_paged_results(self, subsite):
        ...
        # if next_url:
        #     self.fetcher = WordpressCustomFieldsApiFetcher(next_url)
        #     self.get_paged_results(subsite)
    ...
    def stringify_json(self, value):
        # we get json from the api but storing in the db
        # needs a string representation so it stores as a dict
